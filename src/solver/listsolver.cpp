...
z3::check_result listsolver::check_entl() {
        ...
        */
        ...
        get_eq_class(phi_abs, m_ctx.phi_const_vec, phi_eq_class_vec, m_ctx.phi_const_eq_class_vec);

        // logger() << phi_abs << std::endl;

        for (int i=0; i<m_ctx.phi_const_eq_class_vec.size(); i++) {
            logger() << m_ctx.phi_const_vec[i] << ": " << m_ctx.phi_const_eq_class_vec[i] << std::endl;
        }



        construct_graph(m_ctx.phi_const_vec, m_ctx.phi_const_eq_class_vec, phi_eq_class_vec, m_ctx.phi_space, g_phi);
        // g_phi.print(m_ctx.phi_const_vec, m_ctx.phi_space, "g_phi.dot");
        m_ctx.omega_eq_size = phi_eq_class_vec.size();
        /*
        ...
        */
        ...
        get_eq_class(psi_abs, m_ctx.psi_const_vec, psi_eq_class_vec, m_ctx.psi_const_eq_class_vec);
        construct_graph(m_ctx.psi_const_vec, m_ctx.psi_const_eq_class_vec, psi_eq_class_vec, m_ctx.psi_space, g_psi);
...
bool listsolver::is_emp(z3::expr& abs, z3::expr& space) {
    ...
    for (int i=0; i<space.num_args(); i++) {
        ...
        std::string bool_name = logger().string_format("[%s,%d]", source_name.c_str(), i);
...
bool listsolver::check_allocating_plans(graph& g_phi, graph& g_psi, z3::expr& phi_abs) {
        std::vector<int> cc_cycle_num = g_phi.get_cc_cycle_num();
        ...
        z3::expr omega_phi_abs_i = m_ctx.phi_abs;
        ...
        do{
                ...
                if (cc_cycle_total_num > 0) {
                    ...
                    if (ss.check() == z3::sat) {
                            ...
                            logger() << "feasible allocating plan!\n";
                            std::vector<std::set<int> > omega_eq_class_vec;
                            get_eq_class(omega_phi_abs_i, m_ctx.phi_const_vec, omega_eq_class_vec, m_ctx.phi_const_eq_class_vec);
                            m_ctx.omega_eq_size = omega_eq_class_vec.size();
                            ...
                            construct_graph(m_ctx.phi_const_vec, m_ctx.phi_const_eq_class_vec, omega_eq_class_vec, m_ctx.phi_space, omega_g_i1);
                        ...
                        is_feasible = false;
                        logger() << "unfeasible allocating plan!\n";
                    }
                }
...
bool listsolver::match_pto(z3::expr& psi_atom, z3::expr& omega_atom) {
        ...
        if (eq_items.size() > 0) {
            z3::expr check_f = m_ctx.phi_abs && !z3::mk_and(eq_items);
            z3::solver s(z3_ctx());
            if (s.check() == z3::sat) {
                return false;
            }
                ...
                if (expr_tool::is_fun(psi_atom, "pto")) {
                        ...
                        if (cc_cycle_num[cc_id] == 0) {
                            ...
                            logger() << "omega graph is dag-like. \n";
                                ...
                                std::vector<graph::edge_t> edge_cycle = omega_g_i.get_edge_cycle(coord);

                                logger() << "paths size: " << paths.size() << std::endl;
                                // match paths
                                ...
                                for (int j=0; j<edge_cycle.size(); j++) {
                                        logger() <<"cycle edge: "<< edge_cycle[j].first.first << "--" << (edge_cycle[j].second) << "--" << edge_cycle[j].first.second << std::endl;
                                }
...
bool listsolver::match_path_to_atom_space(std::vector<int> &paths, z3::expr &psi_atom) {
        ...
        if (paths.size() == 0) {
            ...
            logger() << "phi_p: " << phi_p << std::endl;
            z3::expr_vector data_pars(z3_ctx());
            pred.get_data_pars(data_pars);
            logger() << "data pars: " << data_pars << std::endl;

            int psi_arg_size = psi_atom.num_args();
            if (paths.size() == 1) {
                // pred -> pto or pred
                z3::expr phi_atom = m_ctx.phi_space.arg(paths[0]);
                ...
                if (expr_tool::is_fun(phi_atom, "pto")) {
                    ...
                    if (phi_vars.size() < expected_size) {
                        for (int j=phi_vars.size(); j<expected_size; j++) {
                            std::string new_data_name = logger().string_format("_data_%d", counter++);
                ...
                for (int i=0; i<paths.size()-1; i++) {
                    z3::expr atom1 = m_ctx.phi_space.arg(paths[i]);
                    z3::expr atom2 = m_ctx.phi_space.arg(paths[i+1]);
                    ...
                    logger() << "atom1: " << atom1 << std::endl;
                    logger() << "atom2: " << atom2 << std::endl;
                    logger() << "vec1:  ";
                    for (int j=0; j<var_vec1.size(); j++) logger() << var_vec1[j] << " ";
                    logger() << std::endl;
                    logger() << "vec2:  ";
                    for (int j=0; j<var_vec2.size(); j++) logger() << var_vec2[j] << " ";
                    logger() << std::endl;
                    ...
                    if (is_pto1) {
                        ...
                        if (expr_tool::is_location(var_vec2[1])) {
                                // dll
                        ...
                        for (int j2=var_vec2.size()-1, j1=var_vec1.size()-1; j2>=1; j2--, j1--) {
                            logger() << var_vec1[j1] << " " << var_vec2[j2] << std::endl;
                ...
                // head and tail
                z3::expr atom2 = m_ctx.phi_space.arg(paths[paths.size()-1]);
                std::vector<z3::expr> var_vec2;
                bool is_pto2 = expr_tool::is_fun(atom2, "pto");


                 for (int j2=head_vec.size()-1, j1=psi_atom.num_args()/2-1; j2>=1; j2--, j1--) {
                        logger() << head_vec[j2] << " " << psi_atom.arg(j1) << std::endl;
                ...
                if (is_pto2) {
                    ...
                    for (int j2=var_vec2.size()-1, j1=psi_arg_size-1; j2>=1; j2--, j1--) {
                            logger() << var_vec2[j2] << " " << psi_atom.arg(j1) << std::endl;
            ...
            logger() << "left data items: " << left_data_items << std::endl;
            logger() << "data_items: " << data_items << std::endl;
            // exit(-1);
            if (data_items.size() > 0) {
                z3::expr check_f = m_ctx.phi_abs && z3::mk_and(left_data_items) && !z3::mk_and(data_items);
                z3::solver s(z3_ctx());
                s.add(check_f);
                if (s.check() == z3::sat) {
                    return false;
 ...
 * @param g : the output
 */
void listsolver::construct_graph(std::vector<z3::expr> &phi_const_vec, std::vector<int>& phi_const_eq_class_vec, std::vector<std::set<int> >& eq_class_vec, z3::expr &phi_space, graph &g) {
        ...
        for (int i=0; i<phi_space.num_args(); i++) {
                ...
                if (atom.decl().name().str()!="pto") {
                        ...
                        std::string bool_name = logger().string_format("[%s,%d]", E.to_string().c_str(), i);

                        z3::expr bool_var = z3_ctx().bool_const(bool_name.c_str());

                        ss.reset();
                        ss.add(m_ctx.phi_abs && bool_var);

                        if (ss.check() == z3::unsat) {
                            continue;
                        }
